...
namespace SmartFillMonitor.Services.Plc
{
    public class ModbusRtuTransport : IPlcTransport
    {
        ...
        public async Task ConnectAsync(DeviceSettings settings, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await DisconnectInternalAsync(); //确保在连接前断开任何现有连接
                ...
                _modbusMaster = factory.CreateRtuMaster(streamResource);
        ...
        public void Dispose()
        {
            ...
            {
                LogHelper.Error("释放 Modbus 传输层时断开连接失败。", ex);
            }
            finally
            {
                _disposed = true;
        ...
        private Task DisconnectInternalAsync()
        {
            if (_modbusMaster != null)
            {
                ...
                {
                    LogHelper.Warn($"释放 Modbus 主站对象失败：{ex.Message}");
                }
            ...
            if (_serialPort != null)
            {
                ...
                {
                    LogHelper.Warn($"关闭串口失败：{ex.Message}");
                }
                ...
                {
                    LogHelper.Warn($"释放串口对象失败：{ex.Message}");
                }
